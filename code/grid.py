...
class RoadGrid:
    def __init__(self, label, grid_size):
        ...
        self.yb = yb
        self.yt = yt
        gridSet = set() #所有有MR的grid
        grid_dict = {} #gridSet中所有grid及里面的MR
        for pos in label:
            lon = pos[0]
            lat = pos[1]
...
#             if not grid_dict.has_key((m,n)):
        ...
        self.grids = [(xl+i[0]*width,yb + i[1]*length) for i in grid_dict.keys()] #每个grid的坐标范围（左下角、右上角）
    ...
    def transform(self, label, sparse=True):
        def one_hot(idx, n):
            ...
            a[idx] = 1
            return a
        grid_pos = [self.gridlist.index((int((i[0]-self.xl)/self.width),int((i[1]-self.yb)/self.length))) for i in label] #坐标所在grid的index
        if sparse:
