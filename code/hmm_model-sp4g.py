...
def merge_2g_engpara():
    eng_para = pd.read_csv('4g/siping_4g_new_gongcan.csv', encoding='gbk')
...
data_2g=pd.read_csv("4g/siping_4g_mode.csv")
...
#from grid import RoadGrid
rg = grid.RoadGrid(np.vstack((tr_label_.values, te_label_.values)),90)
...
con_tr_feature.to_csv("4g/conf_tr_sp4g.csv")
con_te_feature.to_csv("4g/conf_te_sp4g.csv")
tr_feature_r.to_csv("4g/total_conf_tr_sp4g.csv")
...
print np.median(error_te), np.mean(error_te), err[int(len(err) * 0.90)]
